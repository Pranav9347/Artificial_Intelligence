...
def minimax(state, is_maximizing, alpha, beta):
    # Base cases:
    ...
    if is_maximizing: # max node
        ...
        for i in range(3):
            for j in range(3):
                if state[i][j] == ' ': # empty cell
                    state[i][j] = 'X'
                    utility = max(utility, minimax(state,False,alpha,beta))
                    state[i][j] = ' '
        ...
        for i in range(3):
            for j in range(3):
                if state[i][j] == ' ': # empty cell
                    state[i][j] = 'O'
                    utility = min(utility, minimax(state,True,alpha,beta))
                    state[i][j] = ' '
    ...
    for i in range(3):
        for j in range(3):
            if state[i][j] == ' ': # empty state
                state[i][j] = 'X'
                child_state_utility = minimax(state,False,-math.inf,math.inf)
                state[i][j] = ' '
